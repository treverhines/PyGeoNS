...
def wen11(sigma,cts):
  ...
  cts [yr] : Characteristic time-scale
  '''
  return gauss.gpiso(rbf.basis.wen11,(0.0,sigma**2,cts),dim=1)
...
def se(sigma,cts):
  ...
  C(t,t') = sigma^2 * exp(-|t - t'|^2/(2*cts^2))
...
def ibm(sigma,t0):
  ...
  s = t - t0  
  C(t,t') = sigma^2 * min(s,s')^2 * (max(s,s') - min(s,s')/3) / 2 
