...
class Test(unittest.TestCase):
  ...
  def test_weighted_mean_zero_length(self):
    # zero length along the axis should return zero mean and inf 
    # uncertanity
    data = np.zeros((0,))  
    sigma = np.zeros((0,))  
    out = pygeons.downsample.weighted_mean(data,sigma)
  ...
  def test_weighted_mean_zero_sigma(self):
    # make sure that values with zero sigma have infinite weight
    data = np.array([3.0,4.0,5.0])
    sigma = np.array([1.0,1.0,0.0])
    out = pygeons.downsample.weighted_mean(data,sigma)
    self.assertTrue(out[0]==5.0)
    self.assertTrue(out[1]==0.0)
