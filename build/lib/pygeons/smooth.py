...
def _reg_matrices(t,x,
  ...
  #Lx = modest.mp.parmap(mappable_diff_matrix,args_maker())
  if Nprocs == 1:
    Lx = map(mappable_diff_matrix,space_args_maker())
    Lt = map(mappable_diff_matrix,time_args_maker())
  else:  
  ...
  # make a time regularization matrix for each station
  #Lt = []
  #for xi in x:
  #  vert,smp = time_cuts.get_vert_smp(xi) 
